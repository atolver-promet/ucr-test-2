 ...
 */
function ucr_umbrella_profile_install_tasks() {
    $tasks = [];

    $tasks['ucr_umbrella_profile_set_default_theme'] = [];
    $tasks['ucr_umbrella_profile_set_file_settings'] = [];
    $tasks['ucr_umbrella_profile_set_user_roles_info'] = [];
    $tasks['ucr_umbrella_profile_set_text_editors'] = [];
    $tasks['ucr_umbrella_profile_set_add_to_any_settings'] = [];
    $tasks['ucr_umbrella_profile_set_menu_links'] = [];
    $tasks['ucr_umbrella_profile_update_core_config'] = [];
    $tasks['ucr_umbrella_profile_update_profile_config'] = [];
    $tasks['ucr_umbrella_profile_set_workflow_options'] = [];
    $tasks['ucr_umbrella_profile_set_layout_builder'] = [];
    $tasks['ucr_umbrella_profile_create_default_pages'] = [];
 ...
 */
function ucr_umbrella_profile_set_default_theme() {
    Drupal::configFactory()
        ->getEditable('system.theme')
        ->set('default', 'ucr_umbrella_theme')
        ->set('admin', 'seven')
        ->save(TRUE);
 ...
 */
function ucr_umbrella_profile_set_file_settings() {
 ...
 */
function ucr_umbrella_profile_set_user_roles_info() {
 ...
 */
function ucr_umbrella_profile_set_text_editors() {
 ...
 */
function ucr_umbrella_profile_set_add_to_any_settings() {
 ...
 */
function ucr_umbrella_profile_set_menu_links() {
    // Set intial Home Link
    MenuLinkContent::create(array('title' => 'Home', 'link' => array('uri' => 'internal:/'), 'menu_name' => 'main', 'expanded' => true))
        ->save();

    // Set the Audience Link
    $parent_link = MenuLinkContent::create(array('title' => 'Audience Quicklinks...', 'link' => array('uri' => 'internal:#'), 'menu_name' => 'ucr-audience-menu', 'expanded' => true));
    $parent_link->save();

    MenuLinkContent::create(array('title' => 'Demo Link 1', 'link' => array('uri' => 'internal:#'), 'menu_name' => 'ucr-audience-menu', 'expanded' => false, 'parent' => $parent_link->getPluginId()))
        ->save();
    MenuLinkContent::create(array('title' => 'Demo Link 2', 'link' => array('uri' => 'internal:#'), 'menu_name' => 'ucr-audience-menu', 'expanded' => false, 'parent' => $parent_link->getPluginId()))
        ->save();
    MenuLinkContent::create(array('title' => 'Demo Link 3', 'link' => array('uri' => 'internal:#'), 'menu_name' => 'ucr-audience-menu', 'expanded' => false, 'parent' => $parent_link->getPluginId()))
        ->save();
}
 ...
 */
function ucr_umbrella_profile_update_core_config() {
 ...
 */
function ucr_umbrella_profile_update_profile_config() {
 ...
 */
function ucr_umbrella_profile_set_workflow_options() {
 ...
 */
function ucr_umbrella_profile_set_layout_builder() {
 ...
 */
function ucr_umbrella_profile_create_default_pages() {
    // Create a standard homepage and set the path of it to '/home', with its pathauto turned off.
    $homepage = Node::create(['type' => 'page']);
    $body_homepage = [
        'value' => '<p>This is a welcome message to your brand-new UCR Website!</p>
...
<p>Suspendisse pellentesque leo accumsan, viverra odio sit amet, molestie tortor. Sed in nunc molestie ex venenatis luctus. Praesent eu ipsum metus. Quisque id augue turpis. Nulla facilisi. Phasellus vulputate nec urna at tincidunt. Vivamus eget nulla sed nibh semper molestie a et nunc. Vivamus dignissim est vel felis eleifend, at porta libero pulvinar. Sed porta finibus malesuada. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus sed imperdiet nulla, a dictum tortor.</p>',
    ...
    $homepage->set('title', 'Welcome to Your New Website!');
    $homepage->set('body', $body_homepage);
    $homepage->set('path', ['alias' => '/home', 'pathauto' => false]);
    $homepage->status = 1;
    $homepage->enforceIsNew();
    $homepage->save();
